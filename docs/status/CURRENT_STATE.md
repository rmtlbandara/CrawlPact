...
**Google federated authentication (ADR-0009, not yet deployed)**: implemented locally as an
additional, explicitly authorized customer authentication method alongside passkeys — same
account/session model, `sub`-only identity (never email), no Client Secret, admin sign-in
remains passkey-only with no exception. Full local unit/integration test coverage (real
cryptographic JWT verification against a local test JWKS, real CSRF/state/nonce/replay
protection against a real D1 database) passes; this has **not** been exercised against the real
Google network, preview, or production. See
`docs/security/GOOGLE_AUTHENTICATION_THREAT_REVIEW.md` and
`docs/deployment/GOOGLE_AUTHENTICATION_DEPLOYMENT_CHECKLIST.md` for the full status and remaining
owner/deployment steps. Production and preview remain passkey/recovery-code only until those
steps are completed.
...
| Authentication (Google, ADR-0009) | `code-present-not-production-verified`                    | None — not yet deployed to preview or production                                                                                    | `lib/auth/google*.ts`, `pages/api/auth/google/**`, `pages/api/account/google/**`                            | `google.test.ts` (unit), `google-auth-flow.integration.test.ts` (integration)                                       | Google Auth Platform preview/production origin registration pending | `docs/deployment/GOOGLE_AUTHENTICATION_DEPLOYMENT_CHECKLIST.md` tracks the remaining owner/deployment steps |
