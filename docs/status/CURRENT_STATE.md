...
(never email), no Client Secret, admin sign-in remains passkey-only with no exception. Deployed
to production and preview (commit `b31270d`), where a **real defect was found**: the initial
...
environments. **Corrected same day**: GIS now runs in JavaScript-callback mode
...
`docs/security/GOOGLE_AUTHENTICATION_THREAT_REVIEW.md`. The correction passes full local
unit/integration test coverage (real cryptographic JWT verification against a local test JWKS,
real same-origin/state/nonce/replay protection against a real D1 database) but **has not yet been
redeployed** — production and preview are still running the defective redirect-mode code as of
this pass, meaning Google sign-in does not currently work end-to-end on either. See
`docs/deployment/GOOGLE_AUTHENTICATION_DEPLOYMENT_CHECKLIST.md` for the remaining owner/deployment
steps, including this correction's redeploy.
...
| Authentication (Google, ADR-0009) | `verified-disabled` (deployed, defective; fix not yet redeployed) | Deployed to preview/production (`b31270d`); real cross-site-POST defect confirmed live (Astro rejected it) — see ADR-0009's post-acceptance correction note | `lib/auth/google*.ts`, `pages/api/auth/google/**`, `pages/api/account/google/**`                            | `google.test.ts` (unit), `google-auth-flow.integration.test.ts` (integration) — updated for the corrected transport | Corrected code not yet redeployed; Google Auth Platform preview origin registration still pending | `docs/deployment/GOOGLE_AUTHENTICATION_DEPLOYMENT_CHECKLIST.md` tracks the remaining owner/deployment steps |
