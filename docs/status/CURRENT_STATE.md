...
Last verified: 2026-09-17 (Phase 1 — Growth Control Plane, Repository Reconciliation & Product Polish — complete and deployed; header block re-verified live via `gh api`, the Cloudflare API, and direct D1 reads against production after the owner triggered `deploy-production.yml`, not carried forward from the pre-deployment pass). See `docs/baseline/2026-09-17-growth-control-plane-phase1/PRODUCTION_VALIDATION.md` for the full post-deployment evidence trail.
Repository commit: 9a3f950689e3ff6d9de5065a6f69a0e69b8adc86 (main; PR #202, "Phase 1: Growth Control Plane, Repository Reconciliation & Product Polish" — squash-merged; production Worker runs this exact commit)
Production deployment identifier: crawlpact-web (Cloudflare Worker), https://crawlpact.com and https://app.crawlpact.com (same Worker, two Custom Domains since Phase 3) — Worker version a0847529-571a-4294-997c-634d41b6aac0 (deployed 2026-09-17T10:05:45Z via the guarded deploy-production.yml pipeline, confirmed live via a direct Cloudflare API read 2026-09-17)
Database migration version: 0040_rum_vitals.sql (40/40 applied to production, confirmed via a live D1 read 2026-09-17 — adds gsc_daily_metrics/ga4_daily_metrics/crux_snapshots/rum_vitals, see GOOGLE_GROWTH_CONTROL_PLANE.md and CRUX_AND_RUM_BASELINE.md)
Crawler registry version: 2026.07.3 (active release, unchanged since Phase 17, re-confirmed live 2026-09-17 — freshness re-verified against primary vendor sources for all 9 governed operators this phase; one new crawler, crw_applebot, added to master data but not yet in a published release — see CRAWLER_REGISTRY_FRESHNESS_AUDIT.md. Master crawler count is now 24 across 9 operators.)
