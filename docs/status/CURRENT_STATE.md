...
sign-in and sign-up working on both Preview and Production, 2026-09-07** — the one thing no
amount of automated/synthetic-token testing could substitute for. Account linking, disconnect,
and admin isolation remain covered only by the automated test suites, not yet separately
...
| Authentication (Google, ADR-0009) | `verified-live` (sign-in/sign-up)                         | Corrected transport deployed to preview and production (`72a8630`); product owner manually confirmed real Google sign-in and sign-up working on both environments, 2026-09-07 | `lib/auth/google*.ts`, `pages/api/auth/google/**`, `pages/api/account/google/**`                            | `google.test.ts` (unit), `google-auth-flow.integration.test.ts` (integration) plus this manual real-account confirmation | Account linking, disconnect, and admin isolation are covered by tests but not yet separately confirmed with a real Google account | `docs/deployment/GOOGLE_AUTHENTICATION_DEPLOYMENT_CHECKLIST.md` tracks remaining verification |
