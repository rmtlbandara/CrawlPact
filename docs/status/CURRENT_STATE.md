...
Last verified: 2026-09-17 (Phase 1 — Growth Control Plane, Repository Reconciliation & Product Polish — header block only; re-verified live via `gh api`, the Cloudflare API, and a direct D1 read against production, not carried forward from the prior pass). The header block below reflects the Google Insights release (PR #200/#201); the narrative sections further down were not re-verified phase-by-phase in this pass — see `docs/baseline/2026-09-17-growth-control-plane-phase1/GIT_AND_REPOSITORY_RECONCILIATION.md` for the full evidence trail and what remains unverified.
Repository commit: 5e4f12793cbf657120a3c4b0849d7ad6971caf4e (main; the production Worker runs the build from PR #200, 0ab8b89558db4974c16cf3cab72b109182a245d2 — PR #201 was documentation-only and correctly triggered no redeploy — see the Git/Production reconciliation doc above)
Production deployment identifier: crawlpact-web (Cloudflare Worker), https://crawlpact.com and https://app.crawlpact.com (same Worker, two Custom Domains since Phase 3) — Worker version ba8000c7-f3d2-4914-ad85-2969c1433ca3 (deployed 2026-09-16T05:54:14Z via the guarded deploy-production.yml pipeline, confirmed live via a direct Cloudflare API read 2026-09-17)
Database migration version: 0038_google_oauth.sql (38/38 applied to production, confirmed via a live D1 read 2026-09-17; migration 0039_growth_snapshots.sql exists on the Phase 1 branch but has not yet been deployed)
Crawler registry version: 2026.07.3 (active release, unchanged since Phase 17 — re-confirmed as the only published registry_versions row via a live D1 read 2026-09-17; a full freshness re-verification against primary vendor sources is still outstanding, see docs/registry/CRAWLER_REGISTRY_GOVERNANCE.md and the Phase 15 completion report)
...
**Google federated authentication (ADR-0009)**: an additional, explicitly authorized customer
authentication method alongside passkeys — same account/session model, `sub`-only identity
(never email), no Client Secret, admin sign-in remains passkey-only with no exception. First
deployed to production and preview (commit `b31270d`) with a **real defect**: the initial
implementation configured Google Identity Services in redirect mode, which makes Google itself
POST the credential cross-site to `/api/auth/google` — Astro's own CSRF protection correctly
rejected it (`Cross-site POST form submissions are forbidden`), confirmed live on both
environments. **Corrected same day** (commit `72a8630`): GIS now runs in JavaScript-callback mode
(`ux_mode: "popup"`), so the CrawlPact page's own script makes a same-origin JSON POST instead —
see ADR-0009's post-acceptance correction note and
`docs/security/GOOGLE_AUTHENTICATION_THREAT_REVIEW.md`. Deployed to preview then production, each
independently verified post-deploy (the corrected same-origin request reaching real logic; the
old cross-site request still correctly rejected). **Product owner manually confirmed real Google
sign-up and sign-in working on both Preview and Production, 2026-09-07** — the one thing no
amount of automated/synthetic-token testing could substitute for. **Independently re-verified**
with a direct read-only query against both `crawlpact-db-preview` and `crawlpact-db`
(production): each environment shows a `users` row (`status: active`, `is_admin: 0`) linked to an
`oauth_accounts` row (`provider: google`, `email_verified: 1`), 3 recorded `sessions` for the
account, and **zero admin sessions** — real evidence, not just a self-report, that the account/
link creation, the repeat sign-in, and admin isolation all hold for a real Google account on both
environments. Account linking (connecting Google to an existing passkey account) and the
disconnect lockout safeguard remain covered only by the automated test suites, not yet separately
confirmed with a real Google account — see
`docs/deployment/GOOGLE_AUTHENTICATION_DEPLOYMENT_CHECKLIST.md` for what remains.
...
| Authentication (Google, ADR-0009) | `verified-live` (sign-up/sign-in, incl. admin isolation)  | Corrected transport deployed to preview and production (`72a8630`); product owner manually confirmed real Google sign-up and sign-in on both environments, 2026-09-07, independently re-verified via direct read-only query against `crawlpact-db-preview`/`crawlpact-db`: `users`+`oauth_accounts` rows present on both, 3 sessions per account, 0 admin sessions | `lib/auth/google*.ts`, `pages/api/auth/google/**`, `pages/api/account/google/**`                            | `google.test.ts` (unit), `google-auth-flow.integration.test.ts` (integration) plus this manual real-account confirmation and direct D1 verification | `GOOGLE_CLIENT_ID` configured per environment; Google Identity Services script availability | Account linking (connecting Google to an existing passkey account) and the disconnect lockout safeguard are covered by tests but not yet separately confirmed with a real Google account — see `docs/deployment/GOOGLE_AUTHENTICATION_DEPLOYMENT_CHECKLIST.md` |
