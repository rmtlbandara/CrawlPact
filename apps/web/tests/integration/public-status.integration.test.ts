...
describe("public vs internal status separation (real D1)", () => {
  ...
  it("keeps a single recent webhook failure internal-only — not yet a confirmed pattern", async () => {
    const harness = await createD1TestHarness();
  ...
  it("computes a real Super Admin overview with both public and internal status side by side, plus an internal-warning count the public page never shows", async () => {
    const harness = await createD1TestHarness();
    const isolatedDb = createDb(harness.db);
    try {
      ...
      const monitoringComponent = overview.components.find((c) => c.key === "scheduled_monitoring");
  ...
  it("still escalates the public page for a real active incident, regardless of internal-only signals", async () => {
    const harness = await createD1TestHarness();
    const isolatedDb = createDb(harness.db);
    try {
      const now = new Date().toISOString();
      ...
      expect(billingOverview?.activeIncident?.title).toBe("Checkout failing for new subscriptions");
    } finally {
      await harness.dispose();
    }
  });
