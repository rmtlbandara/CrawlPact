...
import { rollbackRulesetVersion } from "../../../../../../lib/admin/registry";
import { jsonErrorResponse, jsonResponse } from "../../../../../../lib/json-response";

export const prerender = false;

/** POST /api/admin/registry/rulesets/:versionId/rollback — SRS §28.12. */
export const POST: APIRoute = async ({ request, params }) => {
  ...
  try {
    ...
    await requireAdminAction(request, db, {
      action: "registry.ruleset.rollback",
      target: versionId,
      reason: body.data.reason,
      requestId,
    });

    await rollbackRulesetVersion(db, versionId);
