...
import { revokeAllSharedReportsForUser } from "../../../../../lib/admin/users";
import { jsonErrorResponse, jsonResponse } from "../../../../../lib/json-response";

export const prerender = false;

/** POST /api/admin/users/:userId/revoke-shared-reports — SRS §28.3. */
export const POST: APIRoute = async ({ request, params }) => {
  ...
  try {
    ...
    await requireAdminAction(request, db, {
      action: "user.revoke_shared_reports",
      target: userId,
      reason: body.data.reason,
      requestId,
    });

    await revokeAllSharedReportsForUser(db, userId);
